...
} stm32_Mavlink_handle_t;

// mavlink 支持多个通道，使用全局句柄方便对每一个通道配置（通常，一个通道对应一个串口）
extern stm32_Mavlink_handle_t hMAVLink[MAVLINK_COMM_NUM_BUFFERS];
...
static inline void stm32_Mavlink_SEND_UART_BYTES(mavlink_channel_t chan, const uint8_t *buf, uint16_t len)
{
    if (chan < MAVLINK_COMM_NUM_BUFFERS) {
        ...
        // 请检查通道号是否给错
        // 如果当前通道数量不够用，请增大 stm32_mavlink.h 中 MAVLINK_COMM_NUM_BUFFERS 的值
        while (1) {}
    }
}
#define MAVLINK_SEND_UART_BYTES(chan, buf, len) stm32_Mavlink_SEND_UART_BYTES(chan, buf, len)
 ...
 * @param chan 通道号
 */
void stm32_Mavlink_BindChannel(UART_HandleTypeDef *huart, mavlink_channel_t chan);
 ...
 */
int stm32_Mavlink_StartReceiveIT(mavlink_channel_t chan);
 ...
 */
static inline HAL_StatusTypeDef stm32_Mavlink_UART_Receive_IT(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
 ...
 * @param chan
 */
static inline void stm32_Mavlink_UARTRxCpltCallback(UART_HandleTypeDef *huart, mavlink_channel_t chan)
{
    if (chan < MAVLINK_COMM_NUM_BUFFERS) {
        if (huart == hMAVLink[chan].huart) {
            if (mavlink_parse_char(chan, hMAVLink[chan].rx_buffer, &(hMAVLink[chan].msg), &(hMAVLink[chan].status))) {
                stm32_Mavlink_MsgRxCpltCallback(&(hMAVLink[chan].msg));
            }
            stm32_Mavlink_UART_Receive_IT(huart, &(hMAVLink[chan].rx_buffer), 1);
        }
    } else {
        // 如果卡在这里，说明 chan 超过了最大值
        // 请检查通道号是否给错
